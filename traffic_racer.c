...
void collision(int* px, int*py) {
	if(crefresh%13==0) {
		score=score+crefresh-8;
		switch(*px) {
				//to show lane of car
			case 1: {
				colx=1;
		...
		for(i=0; i<12; i++) {
			if( (laneno[i]!=0)&&(laneno[i]==colx) ) {
				...
				if(score>max)
					max=score;
				printf("YIKES! YOU JUST HIT AN OBSTACLE\nYOUR SCORE: %d\nHIGHEST SCORE:%d",score,max);
				hit=1;
				printf("\n\nENTER 's' to play again, any other key to exit: ");
				scanf(" %c",&m);
				if(m!='s'&&m!='S')
					break;
				hit=0;
